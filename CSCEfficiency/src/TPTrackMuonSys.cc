...
TPTrackMuonSys::TPTrackMuonSys(const edm::ParameterSet& Conf) : theDbConditions( Conf, consumesCollector() ) {
   ...
   */
  m_lumiScalers = Conf.getParameter<edm::InputTag>("scalersResults");
  lumiscalers_=consumes<LumiScalersCollection>(m_lumiScalers);
  ...
  nEventsMuons_ok2 = 0;
  nEventsMuons_ok3 = 0;
  nEventsMuons_okgood = 0;
  nEventsMuonsTracks = 0;
  treeCount = 0;
  ...
  Nevents        = fs->make <TTree>("Nevents","/Nevents");
  Nevents        ->Branch("Nevents_all",      &Nevents_all, "Nevents_all/I") ;
  Nevents        ->Branch("Nevents_objects",      &Nevents_objects, "Nevents_objects/I") ;
  Nevents        ->Branch("Nevents_tracks",      &Nevents_tracks, "Nevents_tracks/I") ;
  Nevents        ->Branch("Nevents_muons",      &Nevents_muons, "Nevents_muons/I") ;
  Nevents        ->Branch("Nevents_muons_ok",      &Nevents_muons_ok, "Nevents_muons_ok/I") ;
  Nevents        ->Branch("Nevents_muons_ok2",      &Nevents_muons_ok2, "Nevents_muons_ok2/I") ;
  ...
  LumiEtInst=new vector<Float_t>();
  LumiEtInstErr=new vector<Float_t>();
  LumiEtInstQlty=new vector<Float_t>();
  LumiStartOrbit=new vector<Float_t>();
  LumiNumOrbits=new vector<Float_t>();
  fractNtuple->Branch("LumiInst",       &LumiInst) ;
  fractNtuple->Branch("LumiInstErr",       &LumiInstErr) ;
  fractNtuple->Branch("LumiInstQlty",       &LumiInstQlty) ;
  fractNtuple->Branch("LumiEtInst",       &LumiEtInst) ;
  fractNtuple->Branch("LumiEtInstErr",       &LumiEtInstErr) ;
  fractNtuple->Branch("LumiEtInstQlty",       &LumiEtInstQlty) ;
  fractNtuple->Branch("LumiStartOrbit",       &LumiStartOrbit) ;
  fractNtuple->Branch("LumiNumOrbits",       &LumiNumOrbits) ;
...
#define MakeBranchAllSt(Name,Type,Var) sprintf(BranchName,"%s%d",Name,st+1); \
  ...
  RunInfo = fs->make <TTree>("RunInfo","RunInfo");
...
TPTrackMuonSys::analyze(const edm::Event& event, const edm::EventSetup& setup){
  ...
  Int_t idx=0;
  LumiInst->assign(1,-100.);
  LumiInstErr->assign(1,-100.);
  LumiInstQlty->assign(1,-100.);
  LumiEtInst->assign(1,-100.);
  LumiEtInstErr->assign(1,-100.);
  LumiEtInstQlty->assign(1,-100.);
  LumiStartOrbit->assign(1,-100.);
  LumiNumOrbits->assign(1,-100.);
  ...
  if (lumiScalers->size()) {
    ...
    if (lumisection) {
      ...
      // cout <<"\t======================>  idx= "<<idx<<"\t instlumi= "<<it3->instantLumi()  << endl;
      //      idx++;

      //      LumiInst->assign(1,-100.);

      LumiInst->at(idx) = it3->instantLumi();
      LumiInstErr->at(idx) = it3->instantLumiErr();
      LumiInstQlty->at(idx) = it3->instantLumiQlty();
      LumiEtInst->at(idx) = it3->instantETLumi();
      LumiEtInstErr->at(idx) = it3->instantETLumiErr();
      LumiEtInstQlty->at(idx) = it3->instantETLumiQlty();
      LumiStartOrbit->at(idx) = it3->startOrbit();
      LumiNumOrbits->at(idx) = it3->numOrbits();
  ...
  unsigned int event_number_muon=0;
  unsigned int event_number_muon2=0;
  unsigned int event_number_muon3=0;
  unsigned int event_number_muontracks=0;
  unsigned int event_number_muongood=0;
  ...
  */
  ...
  nEventsObjects++;
  Nevents_objects=nEventsObjects;
  ...
  for (reco::MuonCollection::const_iterator muIter1 = muons->begin(); muIter1 != muons->end(); ++muIter1) {  
    ...
    if (event_number_muon2 != event.id().event()){
      nEventsMuons_ok2++;
      Nevents_muons_ok2=nEventsMuons_ok2;
    }
    event_number_muon2=event.id().event();
    ...
    if (event_number_muon3 != event.id().event()){
      nEventsMuons_ok3++;
      Nevents_muons_ok3=nEventsMuons_ok3;
    }
    event_number_muon3=event.id().event();
    ...
    event_number_muongood=event.id().event();
    ...
    for(reco::TrackCollection::const_iterator itTrack = gTracks->begin(); itTrack != gTracks->end(); itTrack++){
      ...
      event_number_muontracks=event.id().event();
